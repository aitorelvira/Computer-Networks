...
class ProxyManager:
    ...
    def add_admin(self, email, passw):
        ...
        :return: VOID
        """
        admin_dictionary = {'email': email, 'passw': passw}
        f = open('admins')
        f.write(admin_dictionary)
        self.proxy_admins.append(admin_dictionary)
        ProxyManager.list_of_admins(self.proxy_admins, email, passw)

    def list_of_admins(self, admins_list, email, passw):
        """
        ...
        ProxyManager.is_admin()
        return admins_list

    def is_admin(self, email, passw, admins_list):
        """
        ...
        self.passw = passw
        self.admins_list = admins_list
        if {'email': email, 'passw': passw} in admins_list:
            return True
        else:
            return 0
    ...
    def add_site_blocked(self, request):
        ...
        :return: VOID
        """
        f = open('blocked_sites')
        line = f.readline()
        self.sites_blocked(line)
        ProxyManager.get_blocked_site(request)
    ...
    def is_site_blocked(self, request):
        ...
        """
        self.sites_blocked
        if request["url"] in self.sites_blocked:
            return True
        return False
    ...
    def add_manager(self, email, password):
        ...
        :return: 
        """
        manager_dictionary = {'email': email, 'passw': password}
        self.managers_credentials.append(manager_dictionary)
        ProxyManager.is_manager(email,password, self.managers_credentials)

    def is_manager(self, email, password, managers_list):
        """
        ...
        if {'email': email, 'passw': password} in managers_list:
            return True
        else:
            return False
    ...
    def get_cached_resource(self, request):
        ...
        """
        url = request["url"]
        privateMode = request["is_private_mode"]
        f = open('cache.txt')
        if url in f:
            f.close()
            return request
    ...
    def clear_all(self):
        ...
        :return: VOID
        """
        ProxyManager.clear_history()
        ProxyManager.clear_cache()
